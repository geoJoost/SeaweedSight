...
def prompt_sam2(data_dir, model_name, max_frames=None):
    ...
    obj_ids = [1]#[#1, 2, 3, 4, 5]  # Start with object IDs 1 and 2
    ...
    for sam2_video_output in model.propagate_in_video_iterator(inference_session):
        ...
        all_outputs[frame_idx]['points'] = new_points
        all_outputs[frame_idx]['labels'] = new_labels
    ...
    print(f"[INFO] Tracked {len(inference_session.obj_ids):,} objects through {len(all_outputs):,} frames")
...
def segment_frames_sam2(data_dir, model_name, max_frames=None):
    ...
    for frame_idx, pil_image in enumerate(video_frames_inference):
        # Create prompts
        points, labels = create_luminance_prompts(pil_image, num_prompts=10, luminance_percentile=10)
